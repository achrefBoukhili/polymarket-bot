...
const PROFILE_REBUILD_INTERVAL = 100; // Rebuild profiles every 100 markets (increased from 25)

/** Hard cap on the trade-hash dedup set to prevent unbounded growth */
const MAX_SEEN_HASHES = 200_000;
/** Hard cap on the in-flight price cache map */
const MAX_PRICE_CACHE = 2_000;
...
class ApiPool {
  ...
  init(
    builtInUrls: { dataApi: string; gammaApi: string },
    userEndpoints: {
      name: string;
      url: string;
      type: string;
    ...
    logger.info(
      { totalEndpoints: this.endpoints.length, strategy: this.strategy },
      'ApiPool initialised',
    );
  }

  private addEndpoint(cfg: {
    name: string;
    url: string;
  ...
  recordFailure(ep: ApiEndpoint): void {
    ...
    if (ep.consecutiveFailures >= ep.maxConsecutiveFailures) {
      ep.healthy = false;
      logger.warn(
        { endpoint: ep.name, failures: ep.consecutiveFailures },
        'ApiPool: endpoint marked unhealthy',
      );
    }
  }
...
export class WhaleScanner {
  ...
  /* ── Scanned market current prices (marketId → assetId → price) ── */
  private marketCurrentPrices = new Map<string, Map<string, number>>();
  ...
  constructor(db: WhaleDB, config: WhaleTrackingConfig, gammaApi: string, clobApi: string) {
    this.db = db;
    this.config = config;
  ...
  getState(): ScannerState {
    ...
    st.scannerConfig = {
      ...
      maxRequestsPerMinute:
        this.apiPool.effectiveRpm('gamma-api') + this.apiPool.effectiveRpm('data-api'),
  ...
  getResults(): ScannedWhaleProfile[] {
    return this.latestProfiles;
  }

  getClusters(): WhaleCluster[] {
    return this.latestClusters;
  }

  getClusterSignals(): ClusterSignal[] {
    return this.clusterSignals;
  }
  ...
  getCopySimResults(): CopySimResult[] {
    return [...this.copySimResults.values()];
  }

  getCopySimResult(address: string): CopySimResult | undefined {
    return this.copySimResults.get(address.toLowerCase());
  }
  ...
  getApiPoolStatus(): ReturnType<ApiPool['getStatus']> {
    return this.apiPool.getStatus();
  }

  getWalletBalance(address: string): number | undefined {
    return this.walletBalances.get(address.toLowerCase());
  }
    ...
    try {
      ...
      while (hasMore && this.state.status === 'scanning') {
        ...
        logger.info(
          ...
          'Scanner: page processed memory usage',
        );

        /* ── Phase 3: Incrementally rebuild profiles (every N markets) ── */
        if (marketsProcessedThisBatch % PROFILE_REBUILD_INTERVAL === 0) {
        ...
        if (
          marketsProcessedThisBatch >= PROFILE_REBUILD_INTERVAL &&
      ...
      if (marketsProcessedThisBatch === 0 && totalQualifying > 0) {
        ...
        this.db.clearScannedTrades();
        this.marketCurrentPrices.clear();
        ...
        this.batchInProgress = false;
        logger.info(
          { totalProfiles: this.latestProfiles.length, batch: this.state.batchNumber },
          'Scanner: full sweep done — resetting for fresh data',
        );
        return;
      }
      ...
      this.state.perf = {
        ...
        avgFetchLatencyMs:
          this.perfFetchCount > 0 ? Math.round(this.perfFetchLatencySum / this.perfFetchCount) : 0,
        ...
        parallelEfficiency:
          this.perfFetchCount > 0
            ? Math.min(
                1,
                Math.round((marketsProcessedThisBatch / (elapsedSec * concurrency)) * 100) / 100,
              )
            : 0,
  ...
  private rebuildProfiles(): void {
    const profiles = this.profileAddresses();
  ...
  private aggregateTrades(
    ...
    if (currentPrices && currentPrices.size > 0) {
      this.marketCurrentPrices.set(marketId, currentPrices);
      // Rolling eviction: keep the map bounded to MAX_PRICE_CACHE entries
      if (this.marketCurrentPrices.size > MAX_PRICE_CACHE) {
        let toEvict = Math.floor(MAX_PRICE_CACHE / 2);
        for (const key of this.marketCurrentPrices.keys()) {
          this.marketCurrentPrices.delete(key);
          if (--toEvict <= 0) break;
        }
      }
    }

    const toInsert: import('./whale_types').ScannedTrade[] = [];
    ...
    for (const t of trades) {
      ...
      if (t.id) {
        ...
        // Rolling eviction: when the Set exceeds the cap, drop the oldest half
        if (this.seenTradeHashes.size > MAX_SEEN_HASHES) {
          let toEvict = Math.floor(MAX_SEEN_HASHES / 2);
          for (const hash of this.seenTradeHashes) {
            this.seenTradeHashes.delete(hash);
            if (--toEvict <= 0) break;
          }
        }
      }
    ...
    if (toInsert.length > 0) {
      this.db.insertScannedTrades(toInsert);
    }
  }

  /* ━━━━━━━━━━━━━━ Step 3: Profile each address ━━━━━━━━━━━━━━ */

  private profileAddresses(): ScannedWhaleProfile[] {
    ...
    const aggregates = this.db.getScannedTradeAggregates();
    ...
    for (const agg of aggregates) {
      ...
      const addr = agg.address;
      const markets = this.db.getScannedMarketsForAddress(addr);
      ...
      for (const m of markets) {
        ...
        const mTrades = this.db.getScannedTradesForAddressMarket(addr, mktId);
        const currentPrices = this.marketCurrentPrices.get(mktId) ?? new Map<string, number>();

        const mAgg: MarketAgg = {
          question: m.question,
          volumeUsd: m.volumeUsd,
          trades: m.trades,
          firstTradeTs: m.firstTradeTs,
          lastTradeTs: m.lastTradeTs,
          buys: mTrades
            .filter((t) => t.side === 'BUY')
            .map((t) => ({
              price: t.price,
        ...
        const netSide: 'BUY' | 'SELL' | 'NEUTRAL' =
          netBuyVol > netSellVol * 1.2 ? 'BUY' : netSellVol > netBuyVol * 1.2 ? 'SELL' : 'NEUTRAL';

        const mktHoldHrs =
          pnlResult.holdTimesMs.length > 0
            ? pnlResult.holdTimesMs.reduce((a, b) => a + b, 0) /
              pnlResult.holdTimesMs.length /
              3_600_000
            : 0;
      ...
      const estimatedWinRate =
        totalClosedTrades + totalUnrealisedPositions > 0
          ? (totalWins + totalUnrealisedWins) / (totalClosedTrades + totalUnrealisedPositions)
          : 0;
      const combinedPnl = totalEstPnl + totalUnrealisedPnl;
      const estimatedRoi = agg.volumeUsd > 0 ? combinedPnl / agg.volumeUsd : 0;

      const avgHoldMs =
        allHoldTimesMs.length > 0
          ? allHoldTimesMs.reduce((a, b) => a + b, 0) / allHoldTimesMs.length
          : 0;
      const sortedHold = [...allHoldTimesMs].sort((a, b) => a - b);
      const medianHoldMs =
        sortedHold.length > 0 ? sortedHold[Math.floor(sortedHold.length / 2)] : 0;
      ...
      const confidenceScore = Math.min(
        1,
        (1 - 1 / Math.max(sampleSize, 1)) * Math.min(1, markets.length / 3),
      );
      ...
      if (totalClosedTrades > 1 && allPerTradePnls.length > 1) {
        ...
        const variance =
          allPerTradePnls.reduce((a, b) => a + (b - meanPnl) ** 2, 0) / allPerTradePnls.length;
      ...
      const volScore = Math.min(
        100,
        (Math.log10(Math.max(agg.volumeUsd, 1)) / Math.log10(1_000_000)) * 100,
      );
  ...
  private autoPromoteTopWhales(profiles: ScannedWhaleProfile[]): number {
    ...
    for (const profile of eligible.slice(0, maxPerScan)) {
      try {
        ...
        promoted++;
        logger.info(
          { address: profile.address.slice(0, 10) + '…', compositeScore: profile.compositeScore },
          'Scanner auto-promoted whale',
        );
      } catch (err) {
  ...
  private inferTags(agg: AddressAgg, winRate: number, roi: number, avgHoldMs: number): string[] {
    ...
    if (winRate >= 0.7) tags.push('sharp');
    else if (winRate >= 0.55) tags.push('profitable');
    if (roi > 0.2) tags.push('high_roi');
  ...
  private detectBigTrades(): void {
    ...
    for (const agg of aggregates) {
      if (agg.maxSingleTradeUsd >= minUsd) {
        this.bigTradeAddresses.set(agg.address, agg.maxSingleTradeUsd);
      }
    }
    ...
    const crossRefOne = async (profile: ScannedWhaleProfile): Promise<void> => {
      ...
      try {
        ...
        for (const [mktId, trades] of marketTrades) {
          this.aggregateTrades(trades, mktId, `Market ${mktId.slice(0, 8)}…`);
        }
    ...
    for (const [marketId, data] of marketWhales) {
      if (data.addresses.size >= minWhales) {
        ...
        const dominantSide: 'BUY' | 'SELL' | 'MIXED' =
          data.buys > data.sells * 1.5 ? 'BUY' : data.sells > data.buys * 1.5 ? 'SELL' : 'MIXED';
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt === maxRetries) {
          logger.error({ err, attempt }, 'Scanner fetch failed');
          return null;
        }
        await this.sleep(Math.pow(2, attempt) * 1000);
      }
  ...
  private startFastScan(): void {
    ...
    logger.info(
      { intervalMs: cfg.intervalMs, topMarkets: cfg.topMarkets },
      'Fast-scan mode started',
    );
  ...
  private async runFastScan(): Promise<void> {
    if (!this.state.enabled || this.isFastScanning) return;
    this.isFastScanning = true;
    ...
    if (markets.length === 0) {
      ...
      if (!res) return;
      if (ep) this.apiPool.recordSuccess(ep);
      markets = ((await res.json()) as GammaMarket[]).filter(
        (m) => m.acceptingOrders && m.conditionId,
      );
    }
    ...
    if (newBigTrades > 0) {
      this.rebuildProfiles();
      logger.info(
        { newBigTrades, marketsScanned: topN.length },
        'Fast-scan: new big trades detected',
      );
    }
    } finally {
      this.isFastScanning = false;
    }
  }
    ...
    const backfillOne = async (market: GammaMarket): Promise<void> => {
      ...
      try {
        ...
        const fetchPage = async (page: number): Promise<ClobTrade[]> => {
          ...
          if (!tRes) {
            if (tep) this.apiPool.recordFailure(tep);
            return [];
          }
        ...
        this.state.scanProgress = parseFloat(
          ((backfillCompleted / topMarkets.length) * 50).toFixed(1),
        );
  ...
  private generateClusterSignals(): void {
    ...
    for (const cluster of this.latestClusters) {
      ...
      const confidence =
        Math.round(
          (countFactor * 0.3 + scoreFactor * 0.3 + directionFactor * 0.25 + volumeFactor * 0.15) *
            1000,
        ) / 1000;
    ...
    /* Merge with existing signals, expire old ones, cap total size */
    const now = Date.now();
    this.clusterSignals = [
      ...newSignals,
    ...
    ].slice(0, 200); // hard cap — prevents unbounded growth across cycles
  ...
  private buildNetworkGraph(): void {
    ...
    const avgConnectivity = profiles.length > 0 ? edges.length / profiles.length : 0;
  ...
  private runCopySimulations(): void {
    ...
    // Replace entirely each cycle — old addresses must not linger
    this.copySimResults.clear();
  ...
  private async lookupTopWalletBalances(): Promise<void> {
    ...
    const topWhales = this.latestProfiles.filter((p) => p.compositeScore >= 60).slice(0, 10);

    // Replace entirely — stale addresses from previous cycles must not accumulate
