/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   Uses bun:sqlite (high-performance, native Bun).
...
export class WhaleDB {
  ...
  private prepareStatements(): void {
    const sqls: Record<string, string> = {
      addWhale: `
        ...
        RETURNING *
      `,
      insertWhaleMinimal: 'INSERT OR IGNORE INTO whales (address) VALUES (?)',
      getWhale: 'SELECT * FROM whales WHERE id = ?',
      getWhaleByAddress: 'SELECT * FROM whales WHERE address = ?',
      insertTrade: `
      ...
      `,
      insertScannedTrade: `
        INSERT OR IGNORE INTO scanned_trades
          (address, market_id, question, side, price, size, notional_usd, asset_id, ts)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      getScannedTradeAggregates: `
        SELECT
          address,
          COUNT(*) as trades,
          SUM(notional_usd) as volumeUsd,
          MAX(notional_usd) as maxSingleTradeUsd,
          MIN(ts) as firstTradeTs,
          MAX(ts) as lastTradeTs
        FROM scanned_trades
        GROUP BY address
      `,
      getScannedMarketsForAddress: `
        SELECT
        ...
        GROUP BY market_id
      `,
      getScannedTradesForAddressMarket: `
        SELECT * FROM scanned_trades
        WHERE address = ? AND market_id = ?
        ORDER BY ts ASC
      `,
      clearScannedTrades: 'DELETE FROM scanned_trades',
      listWhalesCount: 'SELECT COUNT(*) as cnt FROM whales ',
      listWhales: 'SELECT * FROM whales ',
      deleteWhale: "UPDATE whales SET tracking_enabled = 0, updated_at = datetime('now') WHERE id = ?",
      getWhaleTrades: 'SELECT * FROM whale_trades ',
      getMarketTrades: 'SELECT * FROM whale_trades WHERE market_id = ? ORDER BY ts DESC LIMIT ?',
      getTradeByTradeId: 'SELECT * FROM whale_trades WHERE whale_id = ? AND trade_id = ?',
      getWhaleTradeCount: 'SELECT COUNT(*) as cnt FROM whale_trades ',
      getWhaleVolume: 'SELECT COALESCE(SUM(notional_usd),0) as vol FROM whale_trades ',
      getWhaleDistinctMarkets: 'SELECT COUNT(DISTINCT market_id) as cnt FROM whale_trades ',
      upsertPosition: `
        ...
        ON CONFLICT(whale_id, market_id, outcome) DO UPDATE SET
          ...
          updated_at = datetime('now')
      `,
      getPositions: 'SELECT * FROM whale_positions WHERE whale_id = ? AND net_shares != 0',
      upsertDailyMetrics: `
      ...
      `,
      getDailyMetrics: 'SELECT * FROM whale_metrics_daily ',
      insertSettlementEntry: `
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
      `,
      getSettlementEntries: 'SELECT * FROM whale_settlement_ledger WHERE whale_id = ? ORDER BY open_ts',
      upsertCandidate: `
      ...
      `,
      listCandidates: 'SELECT * FROM whale_candidates ',
      approveCandidate: 'UPDATE whale_candidates SET approved = 1 WHERE address = ?',
      muteCandidate: 'UPDATE whale_candidates SET muted_until = ? WHERE address = ?',
      insertAlert: `
        INSERT INTO alerts (whale_id, type, payload, created_at, delivered, read_at)
        VALUES (?, ?, ?, datetime('now'), 0, NULL)
      `,
      listAlerts: 'SELECT * FROM alerts ',
      markAlertRead: "UPDATE alerts SET read_at = datetime('now') WHERE id = ?",
      markAllAlertsRead: "UPDATE alerts SET read_at = datetime('now') WHERE read_at IS NULL",
      markWhaleAlertsRead: "UPDATE alerts SET read_at = datetime('now') WHERE whale_id = ? AND read_at IS NULL",
      getUnreadAlertCount: 'SELECT COUNT(*) as cnt FROM alerts WHERE read_at IS NULL',
      insertSignal: `
        INSERT INTO signals (type, payload, created_at, cursor_key)
        VALUES (?, ?, datetime('now'), ?)
      `,
      listSignals: 'SELECT * FROM signals ',
      createWatchlist: 'INSERT INTO watchlists (name) VALUES (?)',
      listWatchlists: 'SELECT * FROM watchlists ORDER BY created_at',
      deleteWatchlist: 'DELETE FROM watchlists WHERE id = ?',
      addToWatchlist: 'INSERT OR IGNORE INTO watchlist_items (watchlist_id, whale_id) VALUES (?, ?)',
      removeFromWatchlist: 'DELETE FROM watchlist_items WHERE watchlist_id = ? AND whale_id = ?',
      getWatchlistItems: `
      ...
      `,
      getShadowPortfolio: 'SELECT * FROM shadow_portfolios WHERE whale_id = ?',
      getSettledPnl: 'SELECT COALESCE(SUM(realized_pnl),0) as pnl FROM whale_settlement_ledger ',
      getWinRateTotal: 'SELECT COUNT(*) as cnt FROM whale_settlement_ledger WHERE whale_id = ? AND close_ts IS NOT NULL',
      getWinRateWins: 'SELECT COUNT(*) as cnt FROM whale_settlement_ledger WHERE whale_id = ? AND close_ts IS NOT NULL AND realized_pnl > 0',
    };
  ...
  private getStmt(name: string): Statement {
    const stmt = this.statements.get(name);
    if (!stmt) throw new Error(`Statement ${name} not prepared`);
    return stmt;
  }
  ...
  private migrate(): void {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS scanned_trades (
        ...
        asset_id     TEXT,
        ts           TEXT    NOT NULL,
        UNIQUE(address, market_id, side, price, size, ts)
      );
      ...
      CREATE INDEX IF NOT EXISTS idx_scanned_trades_address    ON scanned_trades(address);
      CREATE INDEX IF NOT EXISTS idx_scanned_trades_market     ON scanned_trades(market_id);
    `);
  }
  ...
  addWhale(
    address: string,
    opts?: { displayName?: string; tags?: string[]; notes?: string },
  ): Whale {
    const row = this.getStmt('addWhale').get(
      address.toLowerCase(),
  ...
  bulkAddWhales(addresses: string[]): Whale[] {
    const insert = this.getStmt('insertWhaleMinimal'); // I need to add this to prepareStatements
  ...
  getWhale(id: number): Whale | undefined {
    const row = this.getStmt('getWhale').get(id) as Record<string, unknown> | undefined;
    return row ? this.rowToWhale(row) : undefined;
  }

  getWhaleByAddress(address: string): Whale | undefined {
    const row = this.getStmt('getWhaleByAddress').get(address.toLowerCase()) as
  ...
  listWhales(opts?: {
    ...
    const countRow = this.db
      .query(`SELECT COUNT(*) as cnt FROM whales ${where}`)
    ...
    const rows = this.db
      .query(`SELECT * FROM whales ${where} ORDER BY ${orderBy} LIMIT ? OFFSET ?`)
  ...
  updateWhale(
    ...
    params.push(id);
    this.db.query(`UPDATE whales SET ${sets.join(', ')} WHERE id = ?`).run(...params);
  }

  deleteWhale(id: number): void {
    // Soft delete: disable tracking and clear from watchlists
    this.db
      .query("UPDATE whales SET tracking_enabled = 0, updated_at = datetime('now') WHERE id = ?")
      .run(id);
  }
  ...
  insertTrade(trade: Omit<WhaleTrade, 'id'>): number {
    const info = this.getStmt('insertTrade').run(
      trade.whaleId,
      trade.tradeId,
      trade.logicalTradeGroupId,
      trade.marketId,
      trade.outcome,
      trade.side,
      trade.price,
      ...
      trade.notionalUsd,
      trade.feeUsd,
      trade.isFeeEstimated ? 1 : 0,
      trade.ts,
      trade.midpointAtFill,
      trade.bestBidAtFill,
      trade.bestAskAtFill,
      trade.slippageBps,
      trade.aggressor,
  ...
  insertTrades(trades: Omit<WhaleTrade, 'id'>[]): number {
    const stmt = this.getStmt('insertTrade');
    let inserted = 0;
  ...
  getWhaleTrades(
    ...
    const rows = this.db
      .query(`SELECT * FROM whale_trades ${where} ORDER BY ts DESC LIMIT ?`)
  ...
  getMarketTrades(marketId: string, opts?: { limit?: number }): WhaleTrade[] {
    const rows = this.db
      .query('SELECT * FROM whale_trades WHERE market_id = ? ORDER BY ts DESC LIMIT ?')
  ...
  getTradeByTradeId(whaleId: number, tradeId: string): WhaleTrade | undefined {
    const row = this.db
      .query('SELECT * FROM whale_trades WHERE whale_id = ? AND trade_id = ?')
  ...
  getWhaleTradeCount(whaleId: number, sinceDate?: string): number {
    if (sinceDate) {
      return (
        this.db.query('SELECT COUNT(*) as cnt FROM whale_trades WHERE whale_id = ? AND ts >= ?').get(
          whaleId,
          sinceDate,
        ) as { cnt: number }
      ).cnt;
    ...
    return (
      this.db.query('SELECT COUNT(*) as cnt FROM whale_trades WHERE whale_id = ?').get(whaleId) as {
        cnt: number;
      }
    ).cnt;
  }
  ...
  getWhaleVolume(whaleId: number, sinceDate?: string): number {
    if (sinceDate) {
      return (
        this.db
          .query('SELECT COALESCE(SUM(notional_usd),0) as vol FROM whale_trades WHERE whale_id = ? AND ts >= ?')
          .get(whaleId, sinceDate) as { vol: number }
      ).vol;
    }
    return (
      this.db
        .query('SELECT COALESCE(SUM(notional_usd),0) as vol FROM whale_trades WHERE whale_id = ?')
        .get(whaleId) as { vol: number }
    ).vol;
  }
  ...
  getWhaleDistinctMarkets(whaleId: number, sinceDate?: string): number {
    if (sinceDate) {
      return (
        this.db
          .query('SELECT COUNT(DISTINCT market_id) as cnt FROM whale_trades WHERE whale_id = ? AND ts >= ?')
          .get(whaleId, sinceDate) as { cnt: number }
      ).cnt;
    }
    return (
      this.db
        .query('SELECT COUNT(DISTINCT market_id) as cnt FROM whale_trades WHERE whale_id = ?')
  ...
  getPositions(whaleId: number): WhalePosition[] {
    const rows = this.db
      .query('SELECT * FROM whale_positions WHERE whale_id = ? AND net_shares != 0')
  ...
  getDailyMetrics(
    ...
    const rows = this.db
      .query(`SELECT * FROM whale_metrics_daily ${where} ORDER BY date`)
  ...
  insertSettlementEntry(e: Omit<SettlementLedgerEntry, 'id'>): number {
    ...
    `,
      ...
      .run(
        ...
        e.qty,
        e.entryPrice,
        e.exitPriceOrSettlement,
        e.realizedPnl,
        e.feeUsd,
        e.method,
        e.isEstimatedFee ? 1 : 0,
      );
    return Number(info.lastInsertRowid);
  }
  ...
  getSettlementEntries(whaleId: number): SettlementLedgerEntry[] {
    const rows = this.db
      .query('SELECT * FROM whale_settlement_ledger WHERE whale_id = ? ORDER BY open_ts')
      .all(whaleId) as Record<string, unknown>[];
    return rows.map((r) => ({
      id: r.id as number,
      whaleId: r.whale_id as number,
  ...
  listCandidates(opts?: {
    ...
    const offset = opts?.offset ?? 0;
    const rows = this.db
      .query(`SELECT * FROM whale_candidates ${where} ORDER BY rank_score DESC LIMIT ? OFFSET ?`)
      .all(...params, limit, offset) as Record<string, unknown>[];
  ...
  approveCandidate(address: string): void {
    this.db
      .query('UPDATE whale_candidates SET approved = 1 WHERE address = ?')
  ...
  muteCandidate(address: string, days = 30): void {
    ...
    this.db
      .query('UPDATE whale_candidates SET muted_until = ? WHERE address = ?')
      .run(until, address.toLowerCase());
  }
  ...
  listAlerts(opts?: {
    ...
    const rows = this.db
      .query(`SELECT * FROM alerts ${where} ORDER BY created_at DESC LIMIT ?`)
      .all(...params, opts?.limit ?? 50) as Record<string, unknown>[];
    return rows.map((r) => this.rowToAlert(r));
  }

  markAlertRead(id: number): void {
    this.db.query("UPDATE alerts SET read_at = datetime('now') WHERE id = ?").run(id);
  }
  ...
  markAllAlertsRead(whaleId?: number): void {
    if (whaleId) {
      this.db
        .query(
          "UPDATE alerts SET read_at = datetime('now') WHERE whale_id = ? AND read_at IS NULL",
        )
        .run(whaleId);
    } else {
      this.db.query("UPDATE alerts SET read_at = datetime('now') WHERE read_at IS NULL").run();
    }
  }

  getUnreadAlertCount(): number {
    const row = this.db
      .query('SELECT COUNT(*) as cnt FROM alerts WHERE read_at IS NULL')
      .get() as { cnt: number };
    return row.cnt;
  }
  ...
  listSignals(opts?: { limit?: number; cursor?: string }): WhaleSignal[] {
    ...
    const rows = this.db
      .query(`SELECT * FROM signals ${where} ORDER BY created_at DESC LIMIT ?`)
      .all(...params, opts?.limit ?? 50) as Record<string, unknown>[];
    return rows.map((r) => ({
      id: r.id as number,
  ...
  createWatchlist(name: string): Watchlist {
    const info = this.db.query('INSERT INTO watchlists (name) VALUES (?)').run(name);
    return { id: Number(info.lastInsertRowid), name, createdAt: new Date().toISOString() };
  }

  listWatchlists(): Watchlist[] {
    const rows = this.db.query('SELECT * FROM watchlists ORDER BY created_at').all() as Record<
      string,
      unknown
    >[];
    return rows.map((r) => ({
  ...
  deleteWatchlist(id: number): void {
    this.db.query('DELETE FROM watchlists WHERE id = ?').run(id);
  }

  addToWatchlist(watchlistId: number, whaleId: number): void {
    this.db
      .query('INSERT OR IGNORE INTO watchlist_items (watchlist_id, whale_id) VALUES (?, ?)')
      .run(watchlistId, whaleId);
  }

  removeFromWatchlist(watchlistId: number, whaleId: number): void {
    this.db
      .query('DELETE FROM watchlist_items WHERE watchlist_id = ? AND whale_id = ?')
      .run(watchlistId, whaleId);
  }
  ...
  upsertShadowPortfolio(sp: Omit<ShadowPortfolio, 'id'>): void {
    this.getStmt('upsertShadowPortfolio').run(
      sp.whaleId,
  ...
  getShadowPortfolio(whaleId: number): ShadowPortfolio | undefined {
    const row = this.getStmt('getShadowPortfolio').get(whaleId) as
      | Record<string, unknown>
      | undefined;
  ...
  /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     SCANNED TRADES (Aggregation persistence)
     ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

  insertScannedTrades(trades: Omit<import('./whale_types').ScannedTrade, 'id'>[]): number {
    const stmt = this.getStmt('insertScannedTrade');
    let inserted = 0;
    const trx = this.db.transaction((list: Omit<import('./whale_types').ScannedTrade, 'id'>[]) => {
      for (const t of list) {
        const info = stmt.run(
  ...
  getScannedTradeAggregates(): Array<{
    address: string;
    trades: number;
    volumeUsd: number;
    ...
    return this.getStmt('getScannedTradeAggregates').all() as any[];
  }

  getScannedMarketsForAddress(address: string): Array<{
    marketId: string;
    question: string;
    volumeUsd: number;
    trades: number;
    ...
    return this.getStmt('getScannedMarketsForAddress').all(address.toLowerCase()) as any[];
  }

  getScannedTradesForAddressMarket(
    address: string,
    marketId: string,
  ): import('./whale_types').ScannedTrade[] {
    const rows = this.getStmt('getScannedTradesForAddressMarket').all(
      address.toLowerCase(),
      marketId,
    ) as Record<string, unknown>[];
    return rows.map((r) => ({
  ...
  clearScannedTrades(): void {
    this.getStmt('clearScannedTrades').run();
  }
