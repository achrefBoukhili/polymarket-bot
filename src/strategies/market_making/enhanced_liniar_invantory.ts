...
export class SpreadEnhancedStrategy extends BaseStrategy {
  readonly name = 'market_making_enhanced';
  ...
  override sizePositions(signals: Signal[]): OrderRequest[] {
    ...
    return orders.map((order) => {
      ...
      const inventoryPenalty = Math.max(
        0.3,
        1 - Math.abs(netInventory) / this.maxInventoryPerMarket,
      );
